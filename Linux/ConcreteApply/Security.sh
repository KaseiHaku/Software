...
# 自动关闭空闲连接
# ClientAliveInterval 120     如果超过 120 秒没有接收到 client 的数据，那么发送一个探活心跳。 @trap 不能设置得太久（例如: 20 分钟），因为 防火墙/路由器NAT 的超时时间通常是 300/600 秒，设置太久会被当作空闲连接踢掉
...
# 使用错误密码，检查配置是否生效
shell> ssh -l root -p 22 x.x.x.x       # @trap 一定要新开一个 shell，不新开 shell 锁住就没法用了
