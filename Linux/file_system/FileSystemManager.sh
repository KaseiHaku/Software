...
ls -al -H /path/softlink                            # -H: 如果 ls 的路径是一个 symbolic link，那么展示结果 等价于 对 实际指向的路径 使用 ls 命令，
                                                    # 且结果中的 symbolic link 使用以下格式展示: lrwxrwxrwx ... symbolic_link -> actual_path 
ls -al -L /path/softlink                            # -L: 对 symbolic link 显示文件信息时，实际显示的是 符号链接 指向的文件信息，而不是 符号链接 本身的信息
...
ln -si -T /root/aa ./bb                             # 创建源文件 /root/aa 的符号链接 ./bb，软连接本身是一个真实存在的文件，它有自己的 inodeIndex
                                                    # -i 如果已存在，则打印交互信息
                                                    # -T 即: 永远把 ./bb 作为要创建的 软链接 文件名
ln -si -T -L --backup /root/aa ./bb                 # -L 如果 /root/aa 本身就是软连接，那么需要 解引用 到原始文件

ln -d ./aa.txt ./aa2.txt                            # 创建 硬链接，不允许创建目录的硬链接，硬链接必须在同一文件系统中，硬链接没有自己的 inodeIndex，目标相同的所有硬连接共享一个 inodeIndex
...
tar -tvf archive.tar                                # 不拆包的情况下，查看包内文件                                      
...
tar -zpcf xx1.tar.gz -C ./ ./dir1 ./file2           # 以 ./ 作为 tar 的根目录，将 ./dir1 ./file2 打包成 xx1.tar.gz
tar -zpxf xx1.tar.gz -C ./unTarDir                  # 将 xx1.tar.gz 拆到 ./unTarDir 目录下
