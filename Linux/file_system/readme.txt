...
# 不可与 / 目录挂载在不同分区的目录；
/bin                                    # 存放最经常使用的命令，为 /usr/bin 的软链接
/dev                                    # 设备文件保存目录
/etc                                    # 配置文件保存目录
/lib                                    # 32位依赖库
/lib64                                  # 64位依赖库
/sbin                                   # root 用户才能使用的命令，为 /usr/sbin 的软链接，sudo 命令执行的命令都在该目录下
...
硬盘大小 256G
    方案一：
        /boot/efi           200M                当固件为 UEFI 时，必须存在
        /biosboot           2M                  硬盘采用 GPT 分区，而固件为 BIOS 时，必须存在
        /boot               512M                包含引导系统所需的静态文件，需要在硬盘第一个分区
        /                   64G                 根分区
        swap                16G                 用于内存交换，16G 以上等于内存，否则两倍于内存 
        /opt                128G                测试开发等需要安装的软件比较多时，分大点
        /home               64G                 多用户环境时，分大点
        /tmp                4G                  运行高负载应用时，分大点
        /var                4G                  随着系统的使用，该目录会越来越大，建议分大点，以上目录独立挂载，其他全部挂载在 /var 目录下
    ...
    方案三(Docker)：GUID Partition Table(GPT)
        ...
        /boot               512M                包含引导系统所需的静态文件，需要在硬盘第一个分区
        /                   64G                 根分区
        /opt                96G                 软件盘 or 数据盘；以该顺序分区，后期 opt 可以吞了 home 和 swap
        /home               16G          
        swap                16G                 >=512M, 内存小于 16G，等于两倍内存大小，内存大于 16G 等于内存大小
        /var                ---                 以上目录独立挂载，其他全部挂载在 /var 目录下
