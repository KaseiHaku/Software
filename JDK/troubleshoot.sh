...
################################ Troubleshoot Procedure(无响应/响应慢/死锁/CPU 负载爆表) ################################
...
################################ Troubleshoot Procedure(OOM/频繁 GC) ################################
# 频繁 GC 可能原因:
#   内存过小：-Xmx8g  加内存
#   内存泄漏：

######## 开启 GC 日志
shell> java -Xms512m -Xmx8g                                        # 指定初始内存大小；指定最大内存大小；
            -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=1g         # 指定 Metaspace(永久代) 初始大小；指定 Metaspace(永久代) 上限；
            # GC 日志
            -XX:+PrintGCDetails                    # 开启 GC 日志
            -XX:+PrintGCDateStamps                 # GC 日志中打印时间
            -Xloggc:/var/log/java_gc.log      # 指定 GC 日志输出位置
            -XX:+UseGCLogfileRotation -XX:NumberOfGCLogfiles=8 -XX:GCLogfileSize=512K        # 开启 GC 日志滚动（防止磁盘爆满）；配置 GC 日志文件数量；指定每个 GC 日志文件的大小
            -XX:+PrintGCApplicationStoppedTime     # 打印 GC 造成应用暂停的时间
            -XX:+PrintHeapAtGC                     # 在进行 GC 的前后打印出堆的信息
            # Heap
            -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/var/log/java_pid%p.hprof        # 在 OOM 时导出 heap；配置 HeapDump 位置

######## 导出 Heap
shell> jmap -F -dump:format=b,file=./java_pid<pid>.hprof <pid>        # pid 为 java 启动进程的 PID

######## 分析
shell> # 使用 VisualVM 打开 java_pidxxx.hprof 文件
# 以下适合 jvm 还活着时，进行监控
shell> jmap -heap pid
shell> jmap -histo:live pid
shell> jmap -histo:live,file=./java_jmap.histo pid                    # 生成堆直方图(histogram)，方便排查内存泄漏问题
shell> jconsole                                                       # GUI 排查界面
