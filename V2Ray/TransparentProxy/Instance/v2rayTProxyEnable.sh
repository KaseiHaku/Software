...
iptables -t mangle -A V2RAY -d 10.0.0.0/8 -j RETURN         # 私有地址
iptables -t mangle -A V2RAY -d 172.16.0.0/12 -j RETURN      # 私有地址
iptables -t mangle -A V2RAY -d 192.168.0.0/16 -p tcp ! --dport 53,5353 -j RETURN # 直连局域网，53 端口除外（因为要使用 V2Ray 解析 DNS），避免 V2Ray 无法启动时无法连网关的 SSH，如果你配置的是其他网段（如 10.x.x.x 等），则修改成自己的
iptables -t mangle -A V2RAY -d 192.168.0.0/16 -p udp ! --dport 53,5353 -j RETURN # 直连局域网，53 端口除外（因为要使用 V2Ray 解析 DNS）
...
iptables -t mangle -A V2RAY -d 10.0.0.0/8 -j RETURN         # 私有地址
iptables -t mangle -A V2RAY -d 172.16.0.0/12 -j RETURN      # 私有地址
iptables -t mangle -A V2RAY_MASK -d 192.168.0.0/16 -p tcp ! --dport 53,5353 -j RETURN # 直连局域网，53 端口除外（因为要使用 V2Ray 解析 DNS）
